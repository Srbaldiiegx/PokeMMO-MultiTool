...
function switchShowcaseTab(tab) {
    ...
    if (importForm) importForm.style.display = tab === 'import' && window.isAdmin ? 'grid' : 'none';
...
function openModal(profile) {
    ...
    if (pList.length === 0) {
        ...
        pList.forEach(pokemon => {
            ...
            card.className = `pokemon-card pokemon-status-${status}${pokemon.isAlpha ? ' alpha-shiny' : ''}`;
            ...
            const alphaIcon = pokemon.isAlpha ? '<img class="alpha-icon alpha-icon-card" src="css/alpha-icon-v2.png" alt="Alfa" title="Shiny Alfa">' : '';
            const secretIcon = pokemon.isSecret ? '<img class="alpha-icon secret-icon-card" src="css/secret-icon.png" alt="Secret" title="Secret Shiny">' : '';
            card.innerHTML = `${controls}${statusOverlay}${alphaIcon}${secretIcon}
...
function openPokemonModal(pokemon) {
    ...
    spriteContainer.classList.toggle('alpha-shiny', Boolean(pokemon.isAlpha));
    spriteContainer.querySelector('.detail-status-overlay')?.remove();
    spriteContainer.querySelector('.alpha-icon-detail')?.remove();
    spriteContainer.querySelector('.secret-icon-detail')?.remove();
    if (pokemon.isAlpha) {
        const alphaIcon = document.createElement('img');
        alphaIcon.className = 'alpha-icon alpha-icon-detail';
        alphaIcon.src = 'css/alpha-icon-v2.png';
        alphaIcon.alt = 'Shiny Alfa';
        alphaIcon.title = 'Shiny Alfa';
        spriteContainer.appendChild(alphaIcon);
    }
    if (pokemon.isSecret) {
        const secretIcon = document.createElement('img');
        secretIcon.className = 'alpha-icon secret-icon-detail';
        secretIcon.src = 'css/secret-icon.png';
        secretIcon.alt = 'Secret Shiny';
        secretIcon.title = 'Secret Shiny';
        spriteContainer.appendChild(secretIcon);
    }
...
function renderShowcase(filter = '') {
    ...
    sorted.forEach(profile => {
        ...
        const buddyAlphaIcon = favorite?.isAlpha ? '<img class="alpha-icon alpha-icon-buddy" src="css/alpha-icon-v2.png" alt="Alfa" title="Shiny Alfa">' : '';
        const buddySecretIcon = favorite?.isSecret ? '<img class="alpha-icon secret-icon-buddy" src="css/secret-icon.png" alt="Secret" title="Secret Shiny">' : '';
        const buddyHtml = favorite ? `<div class="buddy-sprite buddy-status-${buddyStatus}${favorite.isAlpha ? ' alpha-shiny' : ''}"><img src="https://img.pokemondb.net/sprites/black-white/anim/shiny/${cleanBuddyName}.gif" onerror="this.src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/0.png'">${buddyOverlay}${buddyAlphaIcon}${buddySecretIcon}</div>` : '<div class="buddy-sprite"></div>';
        ...
        const customBadges = (profile.badges || []).map(badge => {
            const removeButton = window.isAdmin ? `<button class="delete-badge-btn" title="Eliminar badge" aria-label="Eliminar badge" onclick="event.stopPropagation(); deleteBadge('${profile.id}', '${badge.id}')">×</button>` : '';
...
function editPokemon(profileId, pokemonId) {
    ...
    const statusSelect = document.getElementById('poke-status');
    statusSelect.value = ['active', 'sold', 'miss'].includes(pokemon.status) ? pokemon.status : 'active';
    document.getElementById('poke-is-alpha').checked = Boolean(pokemon.isAlpha);
    document.getElementById('poke-is-secret').checked = Boolean(pokemon.isSecret);
