...
function openModal(profile) {
    ...
    if (pList.length === 0) {
        ...
        pList.forEach(pokemon => {
            ...
            const alphaIcon = pokemon.isAlpha ? '<img class="alpha-icon alpha-icon-card" src="css/alpha-icon-transparent.png" alt="Alfa" title="Shiny Alfa">' : '';
            card.innerHTML = `${controls}${statusOverlay}${alphaIcon}
...
function openPokemonModal(pokemon) {
    ...
    if (pokemon.isAlpha) {
        ...
        alphaIcon.src = 'css/alpha-icon-transparent.png';
...
function renderShowcase(filter = '') {
    ...
    sorted.forEach(profile => {
        ...
        const buddyAlphaIcon = favorite?.isAlpha ? '<img class="alpha-icon alpha-icon-buddy" src="css/alpha-icon-transparent.png" alt="Alfa" title="Shiny Alfa">' : '';
        const buddyHtml = favorite ? `<div class="buddy-sprite buddy-status-${buddyStatus}${favorite.isAlpha ? ' alpha-shiny' : ''}"><img src="https://img.pokemondb.net/sprites/black-white/anim/shiny/${cleanBuddyName}.gif" onerror="this.src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/0.png'">${buddyOverlay}${buddyAlphaIcon}</div>` : '<div class="buddy-sprite"></div>';
