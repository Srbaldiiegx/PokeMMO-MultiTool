...
function csvStatus(value) {
    ...
    if (['miss', 'missed', 'huido', 'muerto', 'huido/muerto'].includes(normalized)) return 'miss';
    return 'active';
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('pokemon-form').addEventListener('submit', async (event) => {
        ...
        const pokemon = {
            ...
            isSecret: document.getElementById('poke-is-secret').checked,
    ...
    document.getElementById('csv-import-form').addEventListener('submit', async (event) => {
        ...
        if (!window.isAdmin) return alert('Solo el administrador puede importar shinys.');
        const file = document.getElementById('csv-import-file').files[0];
        if (!file) return;
        const rows = parseCsv(await file.text());
        if (rows.length < 2) return result.textContent = 'El CSV no contiene filas para importar.';
        const headers = rows.shift().map(header => header.trim().toLowerCase());
        const column = (...names) => headers.findIndex(header => names.includes(header));
        const trainerColumn = column('trainer', 'entrenador', 'username', 'usuario');
        const pokemonColumn = column('pokemon', 'pokémon', 'shiny', 'nombre');
        if (trainerColumn < 0 || pokemonColumn < 0) return result.textContent = 'Faltan las columnas obligatorias trainer y pokemon.';
        const read = (record, ...names) => { const index = column(...names); return index >= 0 ? record[index] || '' : ''; };
        let imported = 0;
        const errors = [];
        const changedProfiles = new Map();
        rows.forEach((record, rowIndex) => {
            const trainer = record[trainerColumn]?.trim(), name = record[pokemonColumn]?.trim();
            const profile = window.showcaseData.find(item => item.username.toLowerCase() === trainer?.toLowerCase());
            if (!trainer || !name || !profile) { errors.push(`Fila ${rowIndex + 2}: entrenador o Pokémon no válido.`); return; }
            profile.pokemons = profile.pokemons || [];
            const favorite = csvBoolean(read(record, 'favorite', 'fav', 'favorito'));
            if (favorite) profile.pokemons.forEach(pokemon => pokemon.isFav = false);
            profile.pokemons.push({
                id: `poke_${Date.now()}_${rowIndex}_${Math.floor(Math.random() * 1000)}`,
                name,
                nature: read(record, 'nature', 'naturaleza') || '---',
                location: read(record, 'location', 'lugar', 'ruta') || 'Desconocido',
                encounters: read(record, 'encounters', 'encuentros', 'huevos') || '0',
                status: csvStatus(read(record, 'status', 'estado')),
                isAlpha: csvBoolean(read(record, 'isalpha', 'alpha', 'alfa')),
                isSecret: csvBoolean(read(record, 'issecret', 'secret')),
                isFav: favorite,
                ivHp: read(record, 'ivhp', 'ps') || '0', ivAtk: read(record, 'ivatk', 'ataque') || '0', ivDef: read(record, 'ivdef', 'defensa') || '0',
                ivSpatk: read(record, 'ivspatk', 'spa') || '0', ivSpdef: read(record, 'ivspdef', 'spd') || '0', ivSpeed: read(record, 'ivspeed', 'velocidad', 'vel') || '0'
            });
            changedProfiles.set(profile.id, profile); imported += 1;
        });
        if (!imported) return result.textContent = errors.join(' ');
        result.textContent = 'Importando…';
        try {
            for (const profile of changedProfiles.values()) await saveProfile(profile);
            event.currentTarget.reset();
            result.textContent = `Importados ${imported} shiny(s).${errors.length ? ` Omitidos ${errors.length}: ${errors.slice(0, 3).join(' ')}` : ''}`;
        } catch (error) {
            result.textContent = 'No se ha podido completar la importación. Revisa las reglas de Firebase y vuelve a intentarlo.';
        }
    });
